...
/*
|--------------------------------------------------------------------------
| LOGIKA FINAL RIWAYAT
|--------------------------------------------------------------------------
| Masuk  = barang yang belum pernah dikirim
| Keluar = barang yang sudah pernah dikirim (permanen keluar)
|
| Jadi:
| - status Sudah diterima tetap MASUK KE RIWAYAT KELUAR
| - tanggal aktivitas barang keluar = tanggal_keluar
|--------------------------------------------------------------------------
*/
...
if ($search_input !== '') {
    ...
    $where[] = "(
        b.no_asset LIKE '%$search%'
        OR b.serial_number LIKE '%$search%'
        OR b.user LIKE '%$search%'
        OR tb.nama_barang LIKE '%$search%'
        OR bm.nama_merk LIKE '%$search%'
        OR st.nama_status LIKE '%$search%'
        OR ba.nama_branch LIKE '%$search%'
        OR bas.nama_branch LIKE '%$search%'
        OR bt.nama_branch LIKE '%$search%'
        OR bp.status_pengiriman LIKE '%$search%'
        OR bp.nomor_resi_keluar LIKE '%$search%'
        OR b.keterangan_masalah LIKE '%$search%'
        OR bp.nama_penerima LIKE '%$search%'
        OR bp.jasa_pengiriman LIKE '%$search%'
    )";
}
...
$totalSemuaQuery = mysqli_query($koneksi, "
    SELECT COUNT(DISTINCT b.id) AS total
    $baseFrom
");
$totalSemua = (int) (mysqli_fetch_assoc($totalSemuaQuery)['total'] ?? 0);

$totalMasukQuery = mysqli_query($koneksi, "
    SELECT COUNT(DISTINCT b.id) AS total
    $baseFrom
    WHERE bp.id_pengiriman IS NULL
");
...
$totalKeluarQuery = mysqli_query($koneksi, "
    SELECT COUNT(DISTINCT b.id) AS total
    $baseFrom
    WHERE bp.id_pengiriman IS NOT NULL
");
...
$query = mysqli_query($koneksi, "
    SELECT DISTINCT
        ...
        bas.nama_branch AS branch_asal_pengiriman,
        bt.nama_branch AS branch_tujuan,
        bp.id_pengiriman,
        bp.tanggal_keluar,
        bp.tanggal_diterima,
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .search-inline .search-input-wrap .form-control {
            height: 100%;
        }
                ...
                <div class="page-shell">
                    ...
                    <div class="page-hero">
                        <div class="hero-content">
                            ...
                            <p class="page-desc">
                                Pantau riwayat perpindahan, status masuk, status keluar, dan aktivitas asset perusahaan
                                dengan tampilan yang lebih rapi, jelas, dan mudah dipahami.
                            </p>
                        </div>
                    </div>
                            ...
                            <div class="row g-3 align-items-end">
                                    ...
                                    <div class="search-inline">
                                        <div class="search-input-wrap">
                                            <input
                                                ...
                                                name="cari"
                                                class="form-control"
                                                placeholder="Cari asset, barang, serial number, cabang, user, resi..."
                                                value="<?= h($search_input) ?>">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-3">
                                    <div class="filter-actions">
                                        ...
                                        <a href="<?= h(build_url()) ?>" class="reset-btn w-100">
                            ...
                            <div class="summary-card">
                                <div class="d-flex justify-content-between align-items-start gap-3">
                                    <div>
                                        ...
                                        <div class="summary-note">Seluruh asset yang tercatat di sistem</div>
                                    </div>
                            ...
                            <div class="summary-card">
                                <div class="d-flex justify-content-between align-items-start gap-3">
                                    <div>
                                        ...
                                        <div class="summary-note">Sudah pernah dikirim, tetap tercatat keluar</div>
                                    </div>
                                    <div class="summary-icon">
                            ...
                            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                                <div>
                                    <div class="table-title">
                                        <i class="<?= h($tableIcon) ?> me-2"></i><?= h($tableTitle) ?>
                                    </div>
                                ...
                                <tbody>
                                    <?php if ($query && mysqli_num_rows($query) > 0): ?>
                                        ...
                                        <?php while ($d = mysqli_fetch_assoc($query)): ?>
                                            <?php
                                            $jenisRiwayat = $d['jenis_riwayat'] ?? 'Masuk';
                                            $statusKirim  = $d['id_pengiriman']
                                                ? ($d['status_pengiriman'] ?: 'Belum dikirim')
                                                : 'Belum dikirim';

                                            $asal = $d['id_pengiriman']
                                                ? ($d['branch_asal_pengiriman'] ?: '-')
                                                : ($d['branch_aktif'] ?: '-');

                                            $tujuan = $d['id_pengiriman']
                                                ? ($d['branch_tujuan'] ?: '-')
                                                : ($d['branch_aktif'] ?: '-');
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="meta-line"><i class="bi bi-geo-alt"></i> <?= h($asal) ?></span>
                                                </td>

                                                <td>
                                                    <span class="meta-line"><i class="bi bi-send"></i> <?= h($tujuan) ?></span>
                                                </td>
                                ...
                                <ul class="pagination mb-0 flex-wrap">
                                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                        <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                                            <a class="page-link" href="<?= build_url([
                                                'page' => $i,
                                                'limit' => $limit,
                                                'cari' => $search_input,
                                                'filter' => $filter,
