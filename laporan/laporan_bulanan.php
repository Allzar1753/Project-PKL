...
function status_badge(string $status): string {
    if ($status === 'Sudah diterima') return '<span class="status-badge status-done">Sudah diterima</span>';
    if ($status === 'Sedang perjalanan') return '<span class="status-badge status-road">Sedang perjalanan</span>';
    if ($status === 'Sedang dikemas') return '<span class="status-badge status-pack">Sedang dikemas</span>';
...
function resolve_period_range(string $periode, string $tahun, string $bulan, string $minggu, string $customAwal, string $customAkhir): array {
    $now = new DateTime(); $label = '';
    if (!preg_match('/^\d{4}$/', $tahun)) $tahun = $now->format('Y');
    if (!preg_match('/^(0[1-9]|1[0-2])$/', $bulan)) $bulan = $now->format('m');

    if ($periode === 'tahun') {
        $startDate = new DateTime($tahun . '-01-01'); $endDate = new DateTime($tahun . '-12-31'); $label = 'Tahun ' . $tahun;
    } elseif ($periode === 'minggu') {
        if (!preg_match('/^(\d{4})-W(\d{2})$/', $minggu, $matches)) {
            $minggu = get_iso_week_default(); preg_match('/^(\d{4})-W(\d{2})$/', $minggu, $matches);
        }
        $startDate = new DateTime(); $startDate->setISODate((int) $matches[1], (int) $matches[2]);
        $endDate = clone $startDate; $endDate->modify('+6 days'); $label = 'Minggu ' . (int) $matches[2] . ' - ' . (int) $matches[1];
    } elseif ($periode === 'custom') {
        if ($customAwal === '' && $customAkhir === '') {
            $startDate = new DateTime($now->format('Y-m-01')); $endDate = new DateTime($now->format('Y-m-t'));
        } else {
            $startDate = new DateTime($customAwal !== '' ? $customAwal : $customAkhir); $endDate = new DateTime($customAkhir !== '' ? $customAkhir : $customAwal);
        }
        ...
        $startDate = new DateTime($tahun . '-' . $bulan . '-01'); $endDate = new DateTime($tahun . '-' . $bulan . '-01');
        $endDate->modify('last day of this month'); $label = 'Bulan ' . $startDate->format('F Y'); $periode = 'bulan';
    }
...
function build_pengiriman_note(array $step): string {
    $parts = [];
    if (!empty($step['jasa_pengiriman'])) $parts[] = 'Dikirim via <b>' . h($step['jasa_pengiriman']) . '</b>';
    if (!empty($step['nomor_resi_keluar'])) $parts[] = 'Nomor resi <b>' . h($step['nomor_resi_keluar']) . '</b>';
    return implode('. ', $parts) . '.';
}

function build_penerimaan_note(array $step): string {
    if (($step['status_pengiriman'] ?? '') !== 'Sudah diterima') return 'Belum ada konfirmasi penerimaan barang.';
    $parts = [];
    if (!empty($step['nama_penerima'])) $parts[] = 'Diterima oleh <b>' . h($step['nama_penerima']) . '</b>';
    else $parts[] = 'Penerimaan sudah dikonfirmasi';
    if (!empty($step['tanggal_diterima'])) $parts[] = 'pada <b>' . h($step['tanggal_diterima']) . '</b>';
    return implode('. ', $parts) . '.';
}
...
if ($isAdmin) { mysqli_stmt_bind_param($stmt, 'ss', $range['start'], $range['end']); } 
else { mysqli_stmt_bind_param($stmt, 'ssi', $range['start'], $range['end'], $myBranchId); }
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

$groupedAssets = []; $totalPengiriman = 0; $totalDiterima = 0; $totalProses = 0;
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!empty($row['id_pengiriman'])) {
        ...
        $totalPengiriman++;
        if (($row['status_pengiriman'] ?? '') === 'Sudah diterima') $totalDiterima++; else $totalProses++;
    }
}
...
<head>
    ...
    <style>
        :root {
            --accent-1: #f59e0b; --accent-2: #f97316; --accent-3: #ffb020; --accent-soft: #fff3e3; --accent-soft-2: #fff8ef;
            --dark-1: #171717; --dark-2: #262626; --dark-3: #3f3f46; --text-main: #202020; --text-soft: #6b7280; --text-faint: #9ca3af;
            --line-1: #eadfce; --line-2: #e4d4bf; --line-3: #f2e7d9;
            --bg-page-top: #fff8f0; --bg-page-mid: #fffaf6; --bg-page-bottom: #ffffff;
            --bg-card: #ffffff; --bg-soft: #fffaf4; --bg-soft-2: #fffbf7;
            --success-bg: #ecfdf3; --success-text: #166534; --success-line: #bbf7d0;
            --warning-bg: #fff7ed; --warning-text: #c2410c; --warning-line: #fed7aa;
            --pending-bg: #fff1dc; --pending-text: #9a5410; --pending-line: #f5c27a;
            --shadow-soft: 0 16px 40px rgba(0, 0, 0, 0.07); --shadow-hover: 0 18px 46px rgba(245, 158, 11, 0.16);
            --radius-xl: 28px; --radius-lg: 22px; --radius-md: 18px; --radius-sm: 14px;
        }
        body { margin: 0; background: radial-gradient(circle at top left, rgba(245, 158, 11, 0.14), transparent 24%), radial-gradient(circle at bottom right, rgba(249, 115, 22, 0.10), transparent 18%), linear-gradient(180deg, var(--bg-page-top) 0%, var(--bg-page-mid) 42%, var(--bg-page-bottom) 100%); font-family: 'Plus Jakarta Sans', sans-serif; color: var(--text-main); }
        .page-wrap { padding: 24px 26px; }
        .page-container { max-width: 1180px; margin: 0 auto; }
        .hero-card { position: relative; overflow: hidden; border-radius: var(--radius-xl); background: linear-gradient(135deg, #1a1a1a 0%, #2c2c2c 34%, #8d5a20 62%, #f08a14 100%); box-shadow: 0 18px 45px rgba(240, 138, 20, 0.18); padding: 1.55rem 1.65rem; margin-bottom: 1.35rem; }
        .hero-card::before { content: ""; position: absolute; width: 240px; height: 240px; border-radius: 50%; background: rgba(255, 255, 255, 0.08); top: -100px; right: -80px; }
        .hero-card::after { content: ""; position: absolute; width: 180px; height: 180px; border-radius: 50%; background: rgba(255, 255, 255, 0.06); left: -50px; bottom: -60px; }
        .hero-content { position: relative; z-index: 2; }
        .page-title { color: #fff; font-size: 1.85rem; font-weight: 800; margin-bottom: .35rem; letter-spacing: -0.02em; }
        .page-subtitle { color: rgba(255, 255, 255, 0.84); margin-bottom: 0; line-height: 1.7; max-width: 780px; font-size: .92rem; }
        .hero-action { border: none; background: #fff; color: #111; font-weight: 800; border-radius: 999px; padding: .82rem 1.2rem; box-shadow: 0 10px 24px rgba(0, 0, 0, 0.12); text-decoration: none; display: inline-flex; align-items: center; cursor: pointer; }
        .hero-action:hover { background: #fff7ef; color: #111; }
        .panel-card, .report-card, .asset-card, .summary-card { background: var(--bg-card); border: 1px solid rgba(245, 158, 11, 0.12); border-radius: 22px; box-shadow: var(--shadow-soft); }
        ...
        .summary-icon { width: 50px; height: 50px; border-radius: 16px; display: inline-flex; align-items: center; justify-content: center; font-size: 1.1rem; flex-shrink: 0; color: #fff; background: linear-gradient(135deg, var(--accent-1), var(--accent-2)); box-shadow: 0 10px 24px rgba(245, 158, 11, 0.20); }
        ...
        .card-head-subtitle { font-size: .84rem; color: rgba(255, 255, 255, 0.82); }
        ...
        .form-control:focus, .form-select:focus { border-color: #f5b55a; box-shadow: 0 0 0 .2rem rgba(245, 158, 11, 0.12); }
        .btn-main { border: none; border-radius: 14px; font-weight: 800; padding: .82rem 1rem; background: linear-gradient(135deg, var(--accent-1), var(--accent-2)); color: #fff; box-shadow: 0 12px 28px rgba(245, 158, 11, 0.22); transition: all .22s ease; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; cursor: pointer;}
        .btn-main:hover { color: #fff; transform: translateY(-1px); filter: brightness(.98); }
        .period-chip { display: inline-flex; align-items: center; gap: .45rem; border-radius: 999px; background: var(--accent-soft); color: #9a5410; border: 1px solid rgba(245, 158, 11, 0.18); padding: .38rem .75rem; font-size: .8rem; font-weight: 800; }
        ...
        .info-box { background: linear-gradient(180deg, #fffaf4 0%, #fff3e7 100%); border: 1px solid rgba(245, 158, 11, 0.14); border-radius: 16px; padding: .95rem 1rem; color: var(--text-soft); font-size: .88rem; line-height: 1.7; margin-bottom: 1rem; }
        .timeline { position: relative; padding-left: 1.2rem; }
        .timeline::before { content: ""; position: absolute; top: 0; bottom: 0; left: .32rem; width: 2px; background: linear-gradient(180deg, rgba(245, 158, 11, 0.30), rgba(249, 115, 22, 0.12)); }
        ...
        .log-note-card { background: linear-gradient(180deg, #fffaf4 0%, #fffdfb 100%); border: 1px solid rgba(245, 158, 11, 0.14); border-radius: 14px; padding: .85rem .9rem; }
        ...
        .status-wait { background: #fff7ed; color: #7c4a13; border: 1px solid rgba(245, 158, 11, 0.24); }
...
<body>
                ...
                <div class="page-wrap">
                    <div class="page-container">
                            ...
                            <div class="hero-content d-flex justify-content-between align-items-start flex-wrap gap-3">
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    ...
                                    <?php if (can('laporan.excel')): ?>
                                        <a href="export_excel.php?<?= http_build_query($_GET) ?>" class="hero-action text-success">
                                    ...
                                    <div class="col-lg-2 filter-bulan">
                                        ...
                                        <select name="bulan" class="form-select">
                                            <?php foreach ($bulanOptions as $num => $label): ?>
                                                <option value="<?= h($num) ?>" <?= $bulan === $num ? 'selected' : '' ?>><?= h($label) ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                        ...
                        <div class="row g-3 mb-4 no-print">
                            <div class="col-md-3"><div class="summary-card"><div class="d-flex justify-content-between align-items-start gap-3"><div><div class="summary-label">Total Asset</div><div class="summary-value"><?= (int) $totalAsset ?></div><div class="summary-note">Asset pada periode ini</div></div><div class="summary-icon"><i class="bi bi-box-seam"></i></div></div></div></div>
                            <div class="col-md-3"><div class="summary-card"><div class="d-flex justify-content-between align-items-start gap-3"><div><div class="summary-label">Total Pengiriman</div><div class="summary-value"><?= (int) $totalPengiriman ?></div><div class="summary-note">Seluruh aktivitas kirim</div></div><div class="summary-icon"><i class="bi bi-truck"></i></div></div></div></div>
                            <div class="col-md-3"><div class="summary-card"><div class="d-flex justify-content-between align-items-start gap-3"><div><div class="summary-label">Sudah Diterima</div><div class="summary-value"><?= (int) $totalDiterima ?></div><div class="summary-note">Pengiriman selesai</div></div><div class="summary-icon"><i class="bi bi-check2-circle"></i></div></div></div></div>
                            <div class="col-md-3"><div class="summary-card"><div class="d-flex justify-content-between align-items-start gap-3"><div><div class="summary-label">Masih Proses</div><div class="summary-value"><?= (int) $totalProses ?></div><div class="summary-note">Belum selesai diterima</div></div><div class="summary-icon"><i class="bi bi-hourglass-split"></i></div></div></div></div>
                        </div>
                            ...
                            <div class="card-body-custom">
                                <?php if (empty($groupedAssets)): ?>
                                    <div class="empty-state">
                                        <i class="bi bi-inbox"></i>Tidak ada data laporan pada periode ini.
                                    </div>
                                <?php else: ?>
                                                ...
                                                <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                                                    <div>
                                                        ...
                                                        <div class="asset-meta">
                                                            Asset: <b><?= h($asset['no_asset'] ?? '-') ?></b> | SN: <b><?= h($asset['serial_number'] ?? '-') ?></b> | PIC: <b><?= h($asset['user'] ?? '-') ?></b>
                                                        </div>
                                                    </div>
                                                    <!-- TOMBOL CETAK 1 ASSET SAJA -->
                                                    <?php if (can('laporan.pdf')): ?>
                                                        <button type="button" class="btn-main no-print" onclick="prosesCetak(this, 'tunggal', <?= (int) $asset['id'] ?>)">
                                                            ...
                                                            <div class="timeline-card">
                                                                ...
                                                                <div class="timeline-body">
                                                                    <div class="detail-grid">
                                                                        <div class="detail-box"><div class="detail-label">Cabang Aktif</div><div class="detail-value"><?= h($asset['branch_aktif'] ?? '-') ?></div></div>
                                                                        <div class="detail-box"><div class="detail-label">Tanggal Masuk</div><div class="detail-value"><?= h($asset['tanggal_kirim'] ?? '-') ?></div></div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                                ...
                                                                <div class="timeline-card">
                                                                    <div class="timeline-head d-flex justify-content-between align-items-start flex-wrap gap-2">
                                                                        <div>
                                                                            ...
                                                                            <div class="timeline-subtitle">Dari <?= h($step['branch_asal_pengiriman'] ?? $asset['branch_aktif'] ?? '-') ?> ke <?= h($step['branch_tujuan'] ?? '-') ?></div>
                                                                        </div>
                                                                        <div><?= status_badge((string) ($step['status_pengiriman'] ?? 'Belum dikirim')) ?></div>
                                                                    </div>
                                                                    <div class="timeline-body">
                                                                        <div class="detail-grid">
                                                                            <div class="detail-box"><div class="detail-label">Dari</div><div class="detail-value"><?= h($step['branch_asal_pengiriman'] ?? $asset['branch_aktif'] ?? '-') ?></div></div>
                                                                            <div class="detail-box"><div class="detail-label">Ke</div><div class="detail-value"><?= h($step['branch_tujuan'] ?? '-') ?></div></div>
                                                                            <div class="detail-box"><div class="detail-label">Tanggal Keluar</div><div class="detail-value"><?= h($step['tanggal_keluar'] ?? '-') ?></div></div>
                                                                            <div class="detail-box"><div class="detail-label">Tanggal Diterima</div><div class="detail-value"><?= h($step['tanggal_diterima'] ?? '-') ?></div></div>
                                                                            <div class="detail-box"><div class="detail-label">Jasa Pengiriman</div><div class="detail-value"><?= h($step['jasa_pengiriman'] ?? '-') ?></div></div>
                                                                            <div class="detail-box"><div class="detail-label">Nomor Resi</div><div class="detail-value"><?= h($step['nomor_resi_keluar'] ?? '-') ?></div></div>
                                                                        </div>
                                                                        <div class="log-note-grid">
                                                                            <div class="log-note-card"><div class="log-note-title">Catatan Pengiriman</div><div class="log-note-text"><?= build_pengiriman_note($step) ?></div></div>
                                                                            <div class="log-note-card"><div class="log-note-title">Catatan Penerimaan</div><div class="log-note-text"><?= build_penerimaan_note($step) ?></div></div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
    ...
    <!-- SCRIPT FILTER & MAGIC POP UP PRINT -->
    <script>
